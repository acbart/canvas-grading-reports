...
from canvas_tools import get_setting, get_courses, download_file
from canvas_tools import from_canvas_date, to_canvas_date
from canvas_tools import yaml_load, load_settings
...
def download_quiz(quiz_id, format, filename, course, ignore):
    ...
    if 'errors' in report:
        ...
        return False
    if 'error_report_id' in report:
        log("Failure:", repr(report))
        return False
    if 'file' not in report:
        pid = report['progress_url'].rsplit('/')[-1]
        success = progress_loop(pid, 5)
        if success:
            report = download_quiz_report(quiz_id, course)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--settings', '-s', help='The settings file to use. Defaults to "settings.yaml". If the file does not exist, it will be created.',
                        default='settings.yaml')
    parser.add_argument('--id', '-i', help='The specific quiz ID to analyze. If not specified, all quizzes are used', default=None)
    parser.add_argument('--format', '-t', help='What format to generate the result into.', choices=['html', 'json', 'raw', 'pdf', 'text', 'yaml'], default='raw')
